...
const EnquiriesRequestedPanel = () => {
  ...
  useEffect(() => {
    if (isError1) {
      ...
      //  const filtered = allAdminPrograms.filter(program => program.Status === "archive");
  ...
  //       `https://api.alnawrasplus.com/api/v1/comment/${selected.id}?type=kc`
  //     );
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      await axios.post("https://api.alnawrasplus.com/api/v1/comment/create", {
        user_id: id,
  ...
  const handleReplySubmit = async (commentId) => {
    ...
    try {
      await axios.post("https://api.alnawrasplus.com/api/v1/reply/create", {
        user_id: id,
        ...
        <div className="w-full rounded-3xl border border-gray-100 bg-white p-4 shadow-[0_14px_35px_rgba(15,23,42,0.06)] sm:p-5 lg:w-1/2">
          <div className="flex justify-between items-start flex-wrap gap-2 mb-4">
            ...
            <div className="flex flex-col items-end">
              <span className="mb-1 rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700">
                Archived
              </span>
