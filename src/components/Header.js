...
export default function Header() {
  ...
  const originalName =
    `${localStorage.getItem("originalFirstName") || ""} ${localStorage.getItem("originalLastName") || ""}`.trim();
  ...
  const handleExitImpersonation = () => {
    ...
    localStorage.setItem(
      "userId",
      localStorage.getItem("originalUserId") || "",
    );
    localStorage.setItem(
      "FirstName",
      localStorage.getItem("originalFirstName") || "",
    );
    localStorage.setItem(
      "LastName",
      localStorage.getItem("originalLastName") || "",
    );
      ...
      {isImpersonating && (
        <div className="flex items-center justify-between bg-orange-500 text-white text-xs font-semibold px-4 py-2 z-50">
          <span>
            ...
            {originalName && (
              <span className="ml-1 opacity-80">(Admin: {originalName})</span>
            )}
          </span>
          <button
            type="button"
